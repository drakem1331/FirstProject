	...
	public void playerInterface () {
		...
		do {
			...
			switch(a) {
				...
				break;
				case "TRAIN":
				MoriaBattleThomas thomas = new MoriaBattleThomas(getDamage(), getMaxHP(), getHealth());
				...
				case "MOVE":
					...
					System.out.println("Location: " + (loc/5) + ", " + (loc%5));
					int random = rng.nextInt();
					if(loc==6) {
						System.out.println("You are at the town center, on the South there is an impenetrable wall of mountains, and to the West an impenetrable wall of trees.\n"
								+ "You may rest and use the shop here.");
					}
					if(loc==7){
						System.out.println("You see a dusty, empty house boarded up");
						if(loc7fight<=1) {
							noWander = true;
							System.out.println("Another wild Infernal attacks!");
							MoriaBattleInfernal infernal = new MoriaBattleInfernal(getDamage(),getMaxHP(),getHealth());
							Boolean survived = infernal.moriaFight();
							if(survived == false) {
							...
							System.out.println("A dusty road crosses to the from West to East, to the North you see rocky hills, and to the South a wall of mountains.");
						}
					if(loc!=6) {
						if(noWander==false) {
							if(random%3==0) {
							...
							int survived = hobgoblin.moriaFight();
									if(survived == 0) {
										openWorld.setLocation(1, 1);
										earnGold(-50);
										player.setStats(15, 300, 300);
